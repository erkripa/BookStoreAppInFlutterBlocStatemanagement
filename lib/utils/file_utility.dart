...
abstract class FileUtility {
  ...
  static Future<File?> compressImage(String path) async {
    ...
    resultFile = xFile != null ? convertToFile(xFile) : null;
  ...
  static Future<File?> selectDocument({List<String>? allowedExtensions}) async {
    final filePicker = FilePicker.platform;
    ...
    final pickedDocument = await filePicker.pickFiles(
      ...
      allowCompression: false,
      type: FileType.custom,
      allowedExtensions: allowedExtensions ?? ['pdf', 'png', 'jpg', 'jpeg'],
    );
